...
\item Um objeto acordável deve fazer duas coisas:
	\begin{itemize}
	\item Pedir para o Timer acordá-lo após certo tempo.
...
\begin{itemize}
\item A interface Wakeable declara o método wakeUp() mas não a implementa. Ela também define constantes úteis.
...
\begin{itemize}
\item Seria a classe abstrata Wakeable abaixo equivalente à interface?
\end{itemize}
\begin{lstlisting}
...
\begin{frame}[fragile]{Interfaces funcionais com classes internas}
    ...
    \begin{enumerate}
        ...
        \item Solução possível: classe \lstinline|OrdenarPorQuantidade| implementa a interface \lstinline|Comparator<T>|.
    \end{enumerate}
...
\begin{frame}[fragile]{Interfaces funcionais com classes anônimas}
    ...
    \begin{enumerate}
        ...
        \item Solução possível: classe interna anônima que implementa a interface \lstinline|Comparator<T>|.
    \end{enumerate}
...
\begin{frame}[fragile]{Interfaces funcionais com expressões lambda}
    ...
    \begin{enumerate}
        ...
        \item Solução possível: expressão lambda que implementa a interface \lstinline|Comparator<T>|.
    \end{enumerate}
